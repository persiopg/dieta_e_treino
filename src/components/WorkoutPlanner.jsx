...
export default function WorkoutPlanner({ 
  ...
  // Estados para o check-in de treino com duração
  const [showCheckInModal, setShowCheckInModal] = useState(false);
  const [pendingCheckInName, setPendingCheckInName] = useState(null);
  ...
  // Controlar o início do processo de check-in
  const triggerCheckIn = (workoutName) => {
    if (workoutName === 'Descanso' || workoutName === null) {
      onWorkoutCheckIn(workoutName, 0);
    } else {
      setCheckInDuration(workoutName === 'Cardio' ? 30 : 60);
      setPendingCheckInName(workoutName);
  ...
  const submitCheckIn = (e) => {
    ...
    onWorkoutCheckIn(pendingCheckInName, checkInDuration);
  ...
  return (
    <div className="w-full max-w-[1600px] mx-auto p-4 md:p-6 space-y-6 pb-24">
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {!isEditingBasePlan ? (
                      <button
                        onClick={() => triggerCheckIn(day.name)}
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-850 pt-4 space-y-4">
              ...
              <div className="space-y-2.5">
                {workout.days[selectedDayIndex].exercises?.map((ex, exIdx) => (
                  <div key={ex.id} className="p-3 bg-zinc-50 dark:bg-zinc-900/40 rounded-xl border border-zinc-100/50 dark:border-zinc-900 space-y-2">
                    <div className="flex justify-between items-center">
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="font-bold text-xs text-zinc-800 dark:text-zinc-200 block">{ex.name}</span>
                          {(ex.rationale || ex.expected_result) && (
                            <button
                              onClick={() => setExpandedExerciseId(expandedExerciseId === ex.id ? null : ex.id)}
                              className="text-zinc-400 hover:text-indigo-500 cursor-pointer p-0.5"
                            >
                    ...
                    {/* Bloco de Detalhes Científicos Expandido */}
                    {expandedExerciseId === ex.id && (
                      <div className="p-2.5 bg-indigo-50/40 dark:bg-indigo-950/10 border border-indigo-150/50 dark:border-indigo-950/20 rounded-lg space-y-2 text-[10px] animate-scale-up">
                        {ex.met && (
                          <div className="flex justify-between font-bold text-indigo-650 dark:text-indigo-400">
                            <span>Esforço Estimado:</span>
                            <span>{ex.met} METs</span>
                          </div>
                        )}
                        {ex.rationale && (
                          <div>
                            <span className="font-extrabold block text-zinc-800 dark:text-zinc-200">🤔 Por que fazer?</span>
                            <span className="text-zinc-550 dark:text-zinc-400">{ex.rationale}</span>
                          </div>
                        )}
                        {ex.expected_result && (
                          <div>
                            <span className="font-extrabold block text-zinc-800 dark:text-zinc-200">🎯 Resultado Esperado:</span>
                            <span className="text-zinc-550 dark:text-zinc-400">{ex.expected_result}</span>
                          </div>
                        )}
                      </div>
                    )}
          ...
          {!loggedWorkoutName ? (
            ...
            <div className="text-center py-12 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-w-[600px] mx-auto">
                ...
                <button
                  onClick={() => triggerCheckIn('Descanso')}
                  className="p-4 bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900/40 dark:hover:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-extrabold text-zinc-800 dark:text-zinc-200 transition-all cursor-pointer flex flex-col items-center gap-1"
                >
              ...
              <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-[#121216] border border-zinc-150 dark:border-zinc-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {loggedWorkoutName !== 'Descanso' && workoutCaloriesToday > 0 && (
                      <span className="text-[10px] text-emerald-600 dark:text-emerald-400 font-extrabold flex items-center gap-1 mt-1">
                        🔥 Gasto Calórico Estimado: {workoutCaloriesToday} Kcal ({workoutDurationToday} min)
                      </span>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {loggedFicha.exercises?.map((ex, exIdx) => (
                      <div key={ex.id} className="p-4 bg-zinc-50/50 dark:bg-[#121216]/50 border border-zinc-100 dark:border-zinc-900 rounded-2xl space-y-3.5 animate-fade-in">
                        <div className="flex justify-between items-center">
                          <div>
                            <div className="flex items-center gap-1.5">
                              <span className="font-extrabold text-xs text-zinc-900 dark:text-zinc-50">{ex.name}</span>
                              {(ex.rationale || ex.expected_result) && (
                                <button
                                  onClick={() => setExpandedExerciseId(expandedExerciseId === ex.id ? null : ex.id)}
                                  className="text-zinc-400 hover:text-indigo-500 cursor-pointer p-0.5"
                                  title={lang === 'pt' ? 'Ver informações e por que fazer' : 'View exercise details'}
                                >
                        ...
                        {/* Bloco de Detalhes Científicos (MET e Justificativa) */}
                        {expandedExerciseId === ex.id && (
                          <div className="p-3 bg-indigo-50/40 dark:bg-indigo-950/10 border border-indigo-100/50 dark:border-indigo-950/20 rounded-xl space-y-2 text-[11px] animate-scale-up">
                            {ex.met && (
                              <div className="flex justify-between font-bold text-indigo-650 dark:text-indigo-400">
                                <span>{lang === 'pt' ? 'Gasto Energético Estimado:' : 'Estimated Effort:'}</span>
                                <span>{ex.met} METs</span>
                              </div>
                            )}
                            {ex.rationale && (
                              <div className="space-y-0.5">
                                <span className="font-extrabold block text-zinc-800 dark:text-zinc-200">
                                  {lang === 'pt' ? '🤔 Por que fazer?' : '🤔 Why do it?'}
                                </span>
                                <span className="text-zinc-650 dark:text-zinc-400">{ex.rationale}</span>
                              </div>
                            )}
                            {ex.expected_result && (
                              <div className="space-y-0.5">
                                <span className="font-extrabold block text-zinc-800 dark:text-zinc-200">
                                  {lang === 'pt' ? '🎯 Resultado Esperado:' : '🎯 Expected Result:'}
                                </span>
                                <span className="text-zinc-650 dark:text-zinc-400">{ex.expected_result}</span>
                              </div>
                            )}
                          </div>
                        )}
      ...
      {/* Modal de confirmação de duração do treino (Check-in com MET) */}
      {showCheckInModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in text-zinc-950 dark:text-zinc-50">
          <div className="bg-white dark:bg-[#0c0c0f] border border-zinc-200 dark:border-zinc-800 rounded-3xl p-6 w-full max-w-sm shadow-xl space-y-4 animate-scale-up">
            <div className="text-center space-y-1">
              ...
              <h3 className="text-md font-extrabold text-zinc-950 dark:text-zinc-50">
                {lang === 'pt' ? 'Treino Concluído!' : 'Workout Done!'}
              </h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400">
                {lang === 'pt' 
                  ? `Registre o tempo de duração da sua ficha "${pendingCheckInName}" para calcularmos o gasto calórico estimado.`
                  : `Log the duration of your "${pendingCheckInName}" workout to calculate energy expenditure.`}
              </p>
            </div>

            <form onSubmit={submitCheckIn} className="space-y-4">
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-bold text-zinc-550 dark:text-zinc-455">
                  <span>{lang === 'pt' ? 'Duração da sessão:' : 'Session duration:'}</span>
                  <span className="font-mono text-indigo-500">{checkInDuration} {lang === 'pt' ? 'minutos' : 'minutes'}</span>
                </div>
                <input
                  type="range"
                  min="15"
              ...
              <div className="flex gap-2.5 pt-2">
                <button
                  type="button"
                  onClick={() => { setShowCheckInModal(false); setPendingCheckInName(null); }}
                  className="flex-1 py-2 border border-zinc-200 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-xl text-xs font-bold text-zinc-550 dark:text-zinc-400 cursor-pointer"
                >
                  {lang === 'pt' ? 'Cancelar' : 'Cancel'}
                </button>
                <button
                  type="submit"
                  className="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-extrabold shadow-md cursor-pointer transition-all"
                >
                  {lang === 'pt' ? 'Confirmar Treino' : 'Confirm Workout'}
                </button>
              </div>
            </form>
          </div>
